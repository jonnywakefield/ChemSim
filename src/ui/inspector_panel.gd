...
func _ready():
	...
	# Wait for the entire scene tree to be loaded and ready.
	await get_tree().root.ready
	
	# Get the actual InteractionManager node from the scene.
	var interaction_manager = get_tree().root.get_node("Main/InteractionManager")
	
	# Connect to the signal on that specific node instance.
	if is_instance_valid(interaction_manager):
		interaction_manager.port_inspect_requested.connect(show_on_port)
