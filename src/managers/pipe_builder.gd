...
func handle_click():
	...
	if current_state == State.IDLE:
		if clicked_port and clicked_port.type == UnitPort.PortType.OUTPUT:
			...
			if not connection_manager.is_in_bounds(entry_pos.x, entry_pos.y) or connection_manager.is_grid_solid(entry_pos): return
...
func _update_path_preview():
	...
	if is_instance_valid(connection_manager.pipe_drawer):
		connection_manager.pipe_drawer.set_preview_path(full_pipe_path, current_preview_path)
...
func _finalize_pipe(end_port: UnitPort):
	...
	if not final_path_segment.is_empty():
		final_path_segment.remove_at(0)
...
func reset_drawing():
	...
	start_port = null
	if is_instance_valid(connection_manager) and is_instance_valid(connection_manager.pipe_drawer):
		connection_manager.pipe_drawer.set_preview_path([], [])
