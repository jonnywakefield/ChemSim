...
func _draw():
	...
	for pipe in pipes:
		if pipe.path.size() > 1:
			for i in range(pipe.path.size() - 1):
				var p1 = Vector2(pipe.path[i]) * PIPE_GRID_SIZE + Vector2.ONE * PIPE_GRID_SIZE / 2
				var p2 = Vector2(pipe.path[i+1]) * PIPE_GRID_SIZE + Vector2.ONE * PIPE_GRID_SIZE / 2
	...
	if full_preview_path.size() > 1:
		for i in range(full_preview_path.size() - 1):
			var p1 = Vector2(full_preview_path[i]) * PIPE_GRID_SIZE + Vector2.ONE * PIPE_GRID_SIZE / 2
			var p2 = Vector2(full_preview_path[i+1]) * PIPE_GRID_SIZE + Vector2.ONE * PIPE_GRID_SIZE / 2
			draw_line(p1, p2, Color.STEEL_BLUE, 3.0)

	# Draw selection highlight
	if selected_object_for_highlight:
		if selected_object_for_highlight is UnitProcess:
			var unit_rect = selected_object_for_highlight.get_node("CollisionShape2D").shape.get_rect()
			draw_set_transform_matrix(selected_object_for_highlight.global_transform)
			draw_rect(unit_rect, Color.YELLOW, false, 2.0)
			draw_set_transform_matrix(Transform2D()) # Reset transform
		elif selected_object_for_highlight is Pipe:
			var pipe_path = selected_object_for_highlight.path
			if pipe_path.size() > 1:
				for i in range(pipe_path.size() - 1):
					var p1 = Vector2(pipe_path[i]) * PIPE_GRID_SIZE + Vector2.ONE * PIPE_GRID_SIZE / 2
					var p2 = Vector2(pipe_path[i+1]) * PIPE_GRID_SIZE + Vector2.ONE * PIPE_GRID_SIZE / 2
...
func add_pipe(pipe: Pipe) -> bool:
	if check_for_overlaps(pipe.path, true):
		return false
...
# --- NEW: Function for InteractionManager to find a pipe at a location ---
